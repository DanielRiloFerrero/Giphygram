...
self.addEventListener('activate', e => {
  ...
  let cleaned = caches.keys().then(keys => {
    keys.forEach(key => {
      if (key !== `static-${version}` && key.match('static-')) return caches.delete(key);
    })
  })
...
const staticCache = (req, cacheName = `static-${version}`) => caches.match(req).then(cachedRes => {
...
// Clear old Giphys from 'giphy' cache
